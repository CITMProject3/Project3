...
bool MeshImporter::ImportUUID(const char * file, const char * path, const char * base_path, std::stack<unsigned int>& uuids)
{
	...
	if (scene != nullptr && scene->HasMeshes())
	{
		...
		MeshImporter::ImportNodeUUID(tmp_node, scene, NULL, file_mesh_directory, objects_created, base_path, root_node, uuids);
...
void MeshImporter::ImportNodeUUID(aiNode * node, const aiScene * scene, GameObject * parent, std::string mesh_file_directory, std::vector<GameObject*>& objects_created, std::string folder_path, Data & root_data_node, std::stack<unsigned int>& uuids)
{
	//Transformation ------------------------------------------------------------------------------------------------------------------
	GameObject* go_root = new GameObject(parent);
	objects_created.push_back(go_root);
	...
	for (int i = 0; i < node->mNumChildren; i++)
		MeshImporter::ImportNodeUUID(node->mChildren[i], scene, go_root, mesh_file_directory, objects_created, folder_path, root_data_node, uuids);
}
