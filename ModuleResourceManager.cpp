...
void ModuleResourceManager::ImportFile(const char * path, string base_dir, string base_library_dir, unsigned int uuid) const
{
	...
	case SOUNDBANK:
		...
		// Same UUID generated by Wwise will be used by the Resource Manager
		string str_path = path;
		size_t slash_pos = str_path.find_last_of('/') + 1;
		size_t dot_pos = str_path.find_last_of('.');
		uuid = std::stoul(str_path.substr(slash_pos, dot_pos));
...
void ModuleResourceManager::SoundbankDropped(const char * path, string base_dir, string base_library_dir, unsigned int id) const
{
	...
	string final_soundbank_path = base_library_dir;
	final_soundbank_path += std::to_string(uuid) + "/";
	App->file_system->GenerateDirectory(final_soundbank_path.data());

	string library_dir = final_soundbank_path;
	final_soundbank_path += std::to_string(uuid) + ".bnk";
