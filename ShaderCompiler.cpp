...
int ShaderCompiler::LoadDefaultTerrainShader()
{
	...
	const GLchar* fragment_code =
		...
		"   float step = 0.001;\n"
		"	float weight[10];\n"
		"	for(int n = 0, n < 10, n++) { weight[n] = 0.0; }\n"
		"	for(float z = step * -10.0f, z <= step * 10.0f, z+= step)\n"
		"	{\n"
		"		for(float x = step * -10.0f, x <= step * 10.0f, x+= step)"
		"		{\n"
		...
		"			float val = dist.x;\n"
		"			if (dist.x > 0.9f && _nTextures > 9) { weight[9] += step / 21.0f; }\n"
		"			else if (dist.x > 0.8f && _nTextures > 8) { weight[8] += step / 21.0f; }\n"
		"			else if (dist.x > 0.7f && _nTextures > 7) { weight[7] += step / 21.0f; }\n"
		"			else if (dist.x > 0.6f && _nTextures > 6) { weight[6] += step / 21.0f; }\n"
		"			else if (dist.x > 0.5f && _nTextures > 5) { weight[5] += step / 21.0f; }\n"
		"			else if (dist.x > 0.4f && _nTextures > 4) { weight[4] += step / 21.0f; }\n"
		"			else if (dist.x > 0.3f && _nTextures > 3) { weight[3] += step / 21.0f; }\n"
		"			else if (dist.x > 0.2f && _nTextures > 2) { weight[2] += step / 21.0f; }\n"
		"			else if (dist.x > 0.1f && _nTextures > 1) { weight[1] += step / 21.0f; }\n"
		"			else { weight[0] += step / 21.0f; }\n"
		"			}\n"
		"		}\n"
		"	}\n"
		"	tex_color += texture(_Texture_9, TexCoord) * weight[9]; }\n"
		"	tex_color += texture(_Texture_8, TexCoord) * weight[8]; }\n"
		"	tex_color += texture(_Texture_7, TexCoord) * weight[7]; }\n"
		"	tex_color += texture(_Texture_6, TexCoord) * weight[6]; }\n"
		"	tex_color += texture(_Texture_5, TexCoord) * weight[5]; }\n"
		"	tex_color += texture(_Texture_4, TexCoord) * weight[4]; }\n"
		"	tex_color += texture(_Texture_3, TexCoord) * weight[3]; }\n"
		"	tex_color += texture(_Texture_2, TexCoord) * weight[2]; }\n"
		"	tex_color += texture(_Texture_1, TexCoord) * weight[1]; }\n"
		"	tex_color += texture(_Texture_0, TexCoord) * weight[0]; }\n"
		"}\n"
		"else\n"
		"{\n"
