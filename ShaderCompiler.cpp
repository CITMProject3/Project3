...
int ShaderCompiler::LoadDefaultTerrainShader()
{
	...
	const GLchar* fragment_code =
		...
		"\n"
		"\n"
		"vec4 tex_color = vec4(0,0,0,1);\n"
		"if(_nTextures > 0)\n"
		"{\n"
		"   float step = 0.001f;\n"
		"	int smoothDist = 2;"
		"	float weight[10];\n"
		"	for(int q = 0; q < int(10); q++) { weight[q] = 0.0; }\n"
		"	for(float z = float(step * -smoothDist); z <= float(step * smoothDist); z+= step)\n"
		"	{\n"
		"		for(float x = float(step * -smoothDist); x <= float(step * smoothDist); x+= step)"
		"		{\n"
		"				vec2 texPicker = terrainCoord;\n"
		"				texPicker.x += x;\n"
		"				texPicker.y += z;\n"
		"				vec4 dist = texture(_TextureDistributor, texPicker);\n"
		"				if (dist.x > 0.9f && _nTextures > 9) { weight[9] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.8f && _nTextures > 8) { weight[8] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.7f && _nTextures > 7) { weight[7] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.6f && _nTextures > 6) { weight[6] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.5f && _nTextures > 5) { weight[5] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.4f && _nTextures > 4) { weight[4] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.3f && _nTextures > 3) { weight[3] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.2f && _nTextures > 2) { weight[2] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else if (dist.x > 0.1f && _nTextures > 1) { weight[1] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"				else { weight[0] += 1.0f / ((smoothDist * 2 + 1) * (smoothDist * 2 + 1)); }\n"
		"		}\n"
		"	}\n"
		"	if(_nTextures > 9) { tex_color += texture(_Texture_9, TexCoord) * (weight[9]); }\n"
		"	if(_nTextures > 8) { tex_color += texture(_Texture_8, TexCoord) * (weight[8]); }\n"
		"	if(_nTextures > 7) { tex_color += texture(_Texture_7, TexCoord) * (weight[7]); }\n"
		"	if(_nTextures > 6) { tex_color += texture(_Texture_6, TexCoord) * (weight[6]); }\n"
		"	if(_nTextures > 5) { tex_color += texture(_Texture_5, TexCoord) * (weight[5]); }\n"
		"	if(_nTextures > 4) { tex_color += texture(_Texture_4, TexCoord) * (weight[4]); }\n"
		"	if(_nTextures > 3) { tex_color += texture(_Texture_3, TexCoord) * (weight[3]); }\n"
		"	if(_nTextures > 2) { tex_color += texture(_Texture_2, TexCoord) * (weight[2]); }\n"
		"	if(_nTextures > 1) { tex_color += texture(_Texture_1, TexCoord) * (weight[1]); }\n"
		"	if(_nTextures > 0) { tex_color += texture(_Texture_0, TexCoord) * (weight[0]); }\n"
		"}\n"
		"else\n"
		"{\n"
