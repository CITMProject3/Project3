...
int ShaderCompiler::LoadDefaultTerrainShader()
{
	...
	const GLchar* fragment_code =
		...
		"	if(dist[0] > 0) {\n"
		"		tex_color += texture(_Texture_0, TexCoord) * float(dist[0]);\n"
		"		}\n"
		"	if(dist[1] > 0) {\n"
		"		tex_color += texture(_Texture_1, TexCoord) * float(dist[1]);\n"
		"		}\n"
		"	if(dist[2] > 0) {\n"
		"		tex_color += texture(_Texture_2, TexCoord) * float(dist[2]);\n"
		"		}\n"
		"	if(dist[3] > 0) {\n"
		"		tex_color += texture(_Texture_3, TexCoord) * float(dist[3]);\n"
		"		}\n"
