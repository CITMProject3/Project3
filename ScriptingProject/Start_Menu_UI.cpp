...
namespace Start_Menu_UI
{
	...
	void Start_Menu_UI_Update(GameObject* game_object)
	{
		...
		{
			// DOWN INPUT ------------------------
			if (App->input->GetJoystickAxis(i, JOY_AXIS::LEFT_STICK_Y) > 0.6 || App->input->GetJoystickButton(i, JOY_BUTTON::DPAD_DOWN) == KEY_DOWN || App->input->GetKey(SDL_SCANCODE_DOWN) )
			{
				if (!down_pressed[i])
				{
					down_pressed[i] = true;
			...
			// UP INPUT ------------------------
			if (App->input->GetJoystickAxis(i, JOY_AXIS::LEFT_STICK_Y) < -0.75 || App->input->GetJoystickButton(i, JOY_BUTTON::DPAD_UP) == KEY_DOWN || App->input->GetKey(SDL_SCANCODE_UP) == KEY_DOWN)
			{
				if (!up_pressed[i])
				{
					up_pressed[i] = true;
