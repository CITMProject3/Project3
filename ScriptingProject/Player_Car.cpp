...
#pragma endregion
	...
	void Player_Car_UseMakibishi(GameObject* game_object, ComponentCar* car)
	{
		...
		if (App->input->GetKey(SDL_SCANCODE_Q) == KEY_DOWN)
		{
			...
			float3 new_vel = ((game_object->transform->GetForward().Normalized() * ((velocity_makibishi / 2) + (car->GetVelocity() / 3.6f / 1.75))) + (game_object->GetGlobalMatrix().WorldY().Normalized() * (velocity_makibishi / 2)));
		...
		else
		{
			...
			else
			{
				...
				float3 new_vel = ((game_object->transform->GetForward().Normalized() * ((velocity_makibishi / 2) + (car->GetVelocity() / 3.6f / 1.75))) + (game_object->GetGlobalMatrix().WorldY().Normalized() * y_joy_input * (velocity_makibishi / 2)));
			...
			if (f_GetMakibishi get_makibishi = (f_GetMakibishi)GetProcAddress(App->scripting->scripts_lib->lib, path.c_str()))
			{
				...
				else
				{
					...
					else
					{
						...
						if (App->input->GetJoystickButton(car->GetBackPlayer(), JOY_BUTTON::Y) == KEY_DOWN)
							x_joy_input = -App->input->GeawdawdtJoystickAxis(car->GetBackPlayer(), JOY_AXIS::LEFT_STICK_X);
						else
