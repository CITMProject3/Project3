...
namespace MapSelectUI
{
	...
	void MapSelectUI_Update(GameObject* game_object)
	{
		...
		for (int playerID = 0; playerID < 4; playerID++)
		{
			...
			if (App->input->GetJoystickAxis(playerID, JOY_AXIS::LEFT_STICK_X) < -0.2 || App->input->GetJoystickAxis(playerID, JOY_AXIS::RIGHT_STICK_X) < -0.2 || App->input->GetJoystickButton(playerID, JOY_BUTTON::DPAD_LEFT) == KEY_DOWN || App->input->GetKey(SDL_SCANCODE_LEFT) == KEY_DOWN)
			{
				// Play Move Selection
				ComponentAudioSource *a_comp = (ComponentAudioSource*)game_object->GetComponent(ComponentType::C_AUDIO_SOURCE);
			...
			if (App->input->GetJoystickAxis(playerID, JOY_AXIS::LEFT_STICK_X) > 0.2 || App->input->GetJoystickAxis(playerID, JOY_AXIS::RIGHT_STICK_X) > 0.2 || App->input->GetJoystickButton(playerID, JOY_BUTTON::DPAD_RIGHT) == KEY_DOWN || App->input->GetKey(SDL_SCANCODE_RIGHT) == KEY_DOWN)
			{
				// Play Move Selection
				ComponentAudioSource *a_comp = (ComponentAudioSource*)game_object->GetComponent(ComponentType::C_AUDIO_SOURCE);
				if (a_comp) a_comp->PlayAudio(1);
